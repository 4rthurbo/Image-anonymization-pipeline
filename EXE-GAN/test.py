...
def eval_(args, generator, device,mask_root,mask_file,eval_dict,):
    ...
    # ---------HIER GROESSE DER MASKE ANPASSEN--------
    #mask_shapes = [args.size//2,args.size//2]
    mask_shapes = [int(args.size * 0.7), int(args.size * 0.7)]
...
def eval_all():
    ...
    for mask_type_ in mask_types:
        ...
        #Korrektur wegen fehlerhaftem Export-Ordner:
        
        #-------------
        if mask_type_.endswith(".txt"):
            eval_dict_ = args.eval_dir
        else:
            eval_dict_ = os.path.join(args.eval_dir, mask_type_)
        #--------------
